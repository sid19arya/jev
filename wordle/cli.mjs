...
// ---------- styling ----------
const esc = s => `\x1b[${s}m`;
const rgb = (r, g, b) => s => `${esc(`38;2;${r};${g};${b}`)}${s}${esc(39)}`;
const bgRgb = (r, g, b) => s => `${esc(`48;2;${r};${g};${b}`)}${s}${esc(49)}`;
const bold = s => `${esc(1)}${s}${esc(22)}`;
const dim = s => `${esc(2)}${s}${esc(22)}`;
const strike = s => `${esc(9)}${s}${esc(29)}`;
const C = {
...
const tile = (letter, state) => TILE_BG[state](bold(C.white(` ${(letter || ' ').toUpperCase()} `)));
const visibleLen = s => s.replace(/\x1b\[[0-9;]*m/g, '').length;
const pad = (s, n) => s + ' '.repeat(Math.max(0, n - visibleLen(s)));
const bar = (p, width = 16, color = C.accent) => {
...
function box(title, lines, width = BOX_WIDTH) {
